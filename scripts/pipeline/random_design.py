...
def generate_random_design_perturbations(
    ...
    if uniform:
        x_y_ratio = (x_high - x_low) / (y_high - y_low) if (y_high - y_low) != 0 else 1.0
        num_y = max(1, int(np.floor(np.sqrt(n_design / x_y_ratio))))
        num_x = max(1, int(np.floor(x_y_ratio * num_y)))
