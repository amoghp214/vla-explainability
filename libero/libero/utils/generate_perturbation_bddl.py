...
def add_distractor(bddl_text, target_workspace=None, z_overrides=None):
    """
    ...
    x_min = round(final_cx - half, 6)
    x_max = round(final_cx + half, 6)
    y_min = round(final_cy - half, 6)
    y_max = round(final_cy + half, 6)
    ...
    if obj_m:
        content = obj_m.group(2)
        last_line = content.rstrip().split('\n')[-1] if content.strip() else ""
        indent = re.match(r'^(\s*)', last_line).group(1) if last_line else "    "
        new_content = content + f"{indent}{new_obj} - {obj_type}\n"
        bddl_text = (bddl_text[:obj_m.start()] + obj_m.group(1) + new_content
                     + obj_m.group(3) + bddl_text[obj_m.end():])
...
def apply_perturbations(bddl_text, perturbations, init_object_range_m=0.0,
                         ...
                         perturbation_spec_dict=None):
    """
    Apply perturbations to any LIBERO scene.
    ...
    if "distractor" in perturbations:
        for _ in perturbations["distractor"]:
            bddl_text = add_distractor(bddl_text, target_workspace, z_overrides=z_overrides)

    bddl_text = fix_init_ranges(bddl_text, init_object_range_m=init_object_range_m,
...
def validate_bddl(bddl_text):
    ...
    region_blocks = find_region_blocks(bddl_text)
    available_regions = set(region_blocks.keys())
    for match in re.finditer(r"\(On\s+\w+\s+([\w_]+)\)", bddl_text):
        full_ref = match.group(1)
        if full_ref in available_regions:
            continue
        if not any(full_ref.endswith(r) for r in available_regions):
            errors.append(f"Region ref '{full_ref}' in :init has no matching region")
